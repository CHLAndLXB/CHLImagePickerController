...
- (void)viewDidLoad {
    ...
    self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:[NSBundle tz_localizedStringForKey:@"Cancel"] style:UIBarButtonItemStylePlain target:self action:@selector(cancel)];
...
- (void)configBottomToolBar {
    ...
    _okButton.frame = CGRectMake(0, 3, 70, 44);
//    _okButton.titleLabel.font = [UIFont systemFontOfSize:16];
    [_okButton addTarget:self action:@selector(okButtonClick) forControlEvents:UIControlEventTouchUpInside];
//    [_okButton setTitle:@"选择" forState:UIControlStateNormal];
//    [_okButton setTitle:@"选择" forState:UIControlStateDisabled];
...
//
    ...
    _numberLable.frame = tzImagePickerVc.selectedModels.count ? CGRectMake(_okButton.frame.size.width-30, 7, 30, 30):CGRectMake(_okButton.frame.size.width, 7, 0, 30);
    ...
    [_okButton addSubview:_numberLable];
    
    UILabel * selectedLabel = [[UILabel alloc]initWithFrame:CGRectMake(CGRectGetMinX(_numberLable.frame)-32-4, 7, 32, 30)];
    selectedLabel.text = @"选择";
    selectedLabel.font = [UIFont systemFontOfSize:15];
    selectedLabel.textColor = [UIColor whiteColor];
    [_okButton addSubview:selectedLabel];
...
- (void)refreshBottomToolBarStatus {
    ...
    _numberLable.hidden = tzImagePickerVc.selectedModels.count <= 0;
    _numberLable.text = [NSString stringWithFormat:@"(%zd)",tzImagePickerVc.selectedModels.count];
    _numberLable.frame = _numberLable.hidden?CGRectMake(_okButton.frame.size.width, 7, 0 ,30):CGRectMake(_okButton.frame.size.width-30, 7, 30, 30);
    for (UIView * view in _okButton.subviews) {
        if (view != _numberLable) {
            view.frame = CGRectMake(CGRectGetMinX(_numberLable.frame)-32-4, 7, 32, 30);
        }
    }
    _originalPhotoButton.enabled = tzImagePickerVc.selectedModels.count > 0;
